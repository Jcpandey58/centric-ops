$exposeLog = "C:\https_config\exposeUrl.log"
$date = Get-Date -Format "dd-MM-yyyy"

function Write-Log {
    param ([string]$message)
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    "$timestamp - $message" | Out-File $exposeLog -Append
}
...
# Add-Content -Path $exposeLog -Value "`n=== Output from: keytool ===`n$RunkeytoolCmd`n"
...
# Add-Content -Path $exposeLog -Value "`n=== Output from: update SSL script ===`n$RunBinCmd`n"
